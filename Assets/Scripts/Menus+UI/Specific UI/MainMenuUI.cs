...
public class MainMenuUI : MonoBehaviour
{
    ...
    void Start()
    {
        menus = new()
        {
            ...
            new MM_Hamsterdle(this),
            new MM_Credits(this)
        };

        moreOptionsUI = menuGroups[0].menuItems[0].GetComponent<MoreOptionsUI>();
    }

    // Called by higher level UI Manager
...
public class MM_Hub : MM_Menu
{
    ...
    {
        ui.menuGroups[0].menuItems[1].SetActive(true); // Enable tap
        ui.menuGroups[0].menuItems[2].SetActive(true); // Enable World Space UI
        ui.moreOptionsUI.interactable = true;
    }

    protected override void OnExit()
    {
        ui.menuGroups[0].menuItems[1].SetActive(false);
        ui.menuGroups[0].menuItems[2].SetActive(false);
...
public class MM_Settings : MM_Menu
{
    ...
    protected override void OnEnter()
    {
        ...
        ui.menuGroups[1].menuItems[0].SetActive(true);
        ui.menuGroups[1].menuItems[1].SetActive(true);
    }

    protected override void OnExit()
    {
        EnterSettingsMenu?.Invoke(false);
        ui.menuGroups[1].menuItems[0].SetActive(false);
        ui.menuGroups[1].menuItems[1].SetActive(false);
        ui.moreOptionsUI.interactable = true;
        ui.moreOptionsUI.ExpandOptions();
    }
}

public class MM_Hamsterdle : MM_Menu
{
    public MM_Hamsterdle(MainMenuUI _ui) : base(_ui) { }
    protected override void OnEnter()
    {
        ui.menuGroups[2].menuItems[0].SetActive(true);
    }

    protected override void OnExit()
    {
        ui.menuGroups[2].menuItems[0].SetActive(false);
        ui.moreOptionsUI.interactable = true;
        ui.moreOptionsUI.ExpandOptions();
    }
}
