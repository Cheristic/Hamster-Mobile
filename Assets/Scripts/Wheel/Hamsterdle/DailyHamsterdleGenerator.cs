...
public class DailyHamsterdleGenerator : MonoBehaviour
{
    ...
    internal WedgeSeedData lastPulled_HamsterdleSeedData;
    ...
    public async Task<WedgeSeedData> RetrieveDailySeed()
    {
        ...
        foreach (var seed in seedExistQuery) // Should only be 1 seed
        {
            ...
            if (lastPulled_HamsterdleSeedData.Number == seed.Number)
            {
                // Seed has already been pulled just return this seed
                workingSeed = lastPulled_HamsterdleSeedData;
            } else
            {
        ...
        // ## note: Use Unity's CDN to provide disconnect if game gets bigger and security is an issue
